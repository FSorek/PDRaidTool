...
namespace PDRaidTool.Utilities
{
    public static class HungarianAlgorithm
    {
        ...
        /// minimized.
        /// </summary>
        /// <param name="entries">A cost matrix; the element at row <em>i</em> and column <em>j</em> represents the cost of
        ...
        public static int[] FindAssignments(this PlayerEntry[,] entries)
        {
            if (entries == null)
                throw new ArgumentNullException("entries");

            var h = entries.GetLength(0);
            var w = entries.GetLength(1);

            for (int i = 0; i < h; i++)
            {
                var min = int.MaxValue;
                for (int j = 0; j < w; j++)
                    min = Math.Min(min, entries[i, j].Preference);
                for (int j = 0; j < w; j++)
                    entries[i, j].Preference -= min;
            }
            ...
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    if (entries[i, j].Preference == 0 && !rowsCovered[i] && !colsCovered[j])
                    {
                        masks[i, j] = 1;
                        rowsCovered[i] = true;
                        colsCovered[j] = true;
            ...
            while (step != -1)
            {
                ...
                {
                    case 1:
                        step = RunStep1(entries, masks, rowsCovered, colsCovered, w, h);
                        break;
                    case 2:
                        step = RunStep2(entries, masks, rowsCovered, colsCovered, w, h, ref pathStart);
                        break;
                    case 3:
                        step = RunStep3(entries, masks, rowsCovered, colsCovered, w, h, path, pathStart);
                        break;
                    case 4:
                        step = RunStep4(entries, masks, rowsCovered, colsCovered, w, h);
                        break;
                }
            }
        ...
        private static int RunStep1(PlayerEntry[,] costs, byte[,] masks, bool[] rowsCovered, bool[] colsCovered, int w, int h)
        {
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
        ...
        private static int RunStep2(PlayerEntry[,] costs, byte[,] masks, bool[] rowsCovered, bool[] colsCovered, int w, int h,
        ...
        private static int RunStep3(PlayerEntry[,] costs, byte[,] masks, bool[] rowsCovered, bool[] colsCovered, int w, int h,
        ...
        private static int RunStep4(PlayerEntry[,] costs, byte[,] masks, bool[] rowsCovered, bool[] colsCovered, int w, int h)
        {
        ...
        private static Location FindZero(PlayerEntry[,] costs, byte[,] masks, bool[] rowsCovered, bool[] colsCovered,
            int w, int h)
        {
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    if (costs[i, j].Preference == 0 && !rowsCovered[i] && !colsCovered[j])
        ...
        private static int FindMinimum(PlayerEntry[,] costs, bool[] rowsCovered, bool[] colsCovered, int w, int h)
        {
            ...
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    if (!rowsCovered[i] && !colsCovered[j])
                        minValue = Math.Min(minValue, costs[i, j].Preference);
                }
            }
